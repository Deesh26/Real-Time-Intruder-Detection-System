...
//		printf("camera: reserved %d/%d\n\r", jpeg_size, MAX_PIC_SIZE);
	...
	FILE * pic_stream = fmemopen(ptr_picture, jpeg_size, "rb");
	...
	if(sizeof(bitmap) == BITMAP_SIZE){
			display_bitmap(bitmap, width, height, comp, 20, 100, 5);
		}else{
			return pdFALSE;
		}
	    //      use the picojpg_test to decode the jpg to bitmap, it will return the width, height, comp and bitmap
